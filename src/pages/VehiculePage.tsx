    ...
    <main className="bg-noir">
      ...
      <section className="px-4 md:px-10 py-16 bg-noir-soft border-y border-gold/10">
        ...
        <div className="max-w-3xl mx-auto border border-gold/20 bg-noir-card">
          {specs.map(([k, v], i) => (
            <div key={k} className={`flex flex-col gap-1 md:flex-row md:justify-between px-4 md:px-6 py-4 md:py-5 ${i < specs.length - 1 ? "border-b border-neutral-800" : ""}`}>
      ...
      <section className="px-4 md:px-10 py-16">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          {equip.map((e) => (
            <div key={e.l} className="border border-neutral-800 hover:border-gold/50 transition-colors bg-noir-card p-4 md:p-6 text-center">
