    ...
    <header className="sticky top-0 z-20 bg-noir/95 backdrop-blur-md border-b border-gold/20">
      <div className="flex items-center justify-between px-4 @lg:px-10 py-4">
        ...
        {!isMobile ? (
          <button
            onClick={() => onNav("devis")}
            className="btn-gold px-5 py-2.5 text-xs uppercase"
          >
            Demander un devis
          </button>
        ) : (
          <button
            ...
            className="w-10 h-10 border border-gold/40 flex flex-col justify-center items-center gap-1.5"
          >
      ...
      {isMobile && open && (
        <div className="border-t border-gold/20 bg-noir-soft">
          {links.map((l) => (
            <button
              key={l.id}
              onClick={() => {
